trident Core version 0.13.2 is now available from:

  <https://download.trident.org/litecoin-0.13.2.1/>
...
ECDSA signatures inside trident transactions now use validation using
...
Previous versions of trident Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of trident Core was to raise the default minimum
relay fee.

trident Core 0.13.2 will have a strict maximum size on the mempool. The
...
trident Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running tridentd without having to do any manual configuration.
...
trident Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. trident Core will relay transactions with
...
entirely in trident Core 0.13, and it is at this time undecided whether the
...
trident Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
trident Core automatically creates a hidden service to listen on, without
manual configuration. trident Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if trident Core is listening, and
...
tridentd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals trident
Core to use floating fees. By default, trident Core will use floating
fees.
...
(default: `0.0002` TDC/kB).

At all times, trident Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TDC.
Furthermore, trident Core will never create transactions paying less than
...
`trident.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29882 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29882 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoinrpc>
        ProxyPass http://127.0.0.1:9882/
        ProxyPassReverse http://127.0.0.1:9882/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass tridentd rpc basic auth
...
Restarting tridentd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in trident.conf
...
For example:
    ...
    $ src/trident-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The trident Core code base has
...
for a new opcode in the trident scripting system that in combination with
...
trident transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of trident Core.
...
The two phases are distinct in the trident-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming trident wallet
  software more difficult and which seriously complicates the design of smart
  contracts for trident.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each trident block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** trident addresses (both P2PKH addresses
  ...
  believed to be provided by trident's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of trident's current ledger (called the UTXO set),
  ...
  note that trident Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  trident users to individually opt-in to almost any change in the trident
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and trident Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the trident
client to trident Core 0.13.2 to signal for segwit support. When a
...
Since trident Core 0.10.0, nodes have defaulted to only relaying and
